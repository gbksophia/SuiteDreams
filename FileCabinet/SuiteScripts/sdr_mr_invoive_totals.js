...
function(search) {
    ...
    function getInputData() {
        var invSearch=search.create({
            ...
            filters : [
                ['type', search.Operator.ANYOF, 'CustInvc'], //search.createFilter({name,operator,value})
                'and',
                ['mainline', search.Operator.IS, true]
            ],
            columns:['entity', 'total'] // search.createColumn({name})
        });
        return invSearch; //시스템으로 search를 리턴한다.
    }
    ...
    function summarize(summary) {
        ...
        log.error('Input Error', summary.inputSummary.error); //error for getInputData
