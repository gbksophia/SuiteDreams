    ...
    function onRequest(context) {
        ...
        if(request.method=='GET'){
            ...
            var form=serverWidget.createForm({
                title: 'Update Employee Notes' //,  //Below Navigation bar...
                // hideNavBar: true optional so change it to false is the same as deleting the code.
            });

            var nameFld= form.addField({
                id:'custpage_sdr_emp_name',
                type: serverWidget.FieldType.TEXT, //free-form text
                label:'Name'
            });
            ...
            }); //해당 필드 display only로 설정.

            empIdFld.updateDisplayType({
                displayType: serverWidget.FieldDisplayType.HIDDEN
            }); //해당 필드 숨김.

            form.addSubmitButton('Continue');  //Submit 버튼 생성. Label 변경 (Save-->Continue)

            response.writePage(form); //Response에 Form을 출력...즉 브라우저에 커스텀폼을 출력.
        } else {  // POST
            //get from the form.
